...
class ModelConfig(StorageConfig):
    """
    ...
    Args:
        ...
        compressor (:obj:`dict`, optional, defaults to :code:`{"complevel" : 0, "complib" : "zlib"}`):
            Model compressor option. It consists of a dict type that has complevel and complib as keys.
    ...
    def _hashmap_transfer(self):
        """
        Get unikey bucket name with `model_name` and `additional`

        Returns:
            :obj: `str`:
        """
        if not isinstance(self.model_name, str):
            raise ValueError("model_name {} is empty or not str type".format(self.model_name))
        if not isinstance(self.additional, dict):
            raise TypeError("additional is not dict type")

        key = self.type + self.model_name + json.dumps(self.additional, indent=4, sort_keys=True)
